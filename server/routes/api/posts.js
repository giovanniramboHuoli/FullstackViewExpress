...
//postman: ferramenta para fazer requisições http.
...
router.delete('/:identificacao', async (req, res) => {  //"'/:identificacao'" pega tudo que vem depois da barra em ".../api/posts" e diz que vai se chamar "id".
    ...
    //await posts.deleteMany({text: req.params.identificacao}); //exemplo adicional
    res.status(200).send();
});

//Função para conectar à nossa coleção de posts (BD)
async function loadPostsCollection (){
    ...
    return client.db('postsquerocar').collection('posts'); //pegar a coleção de posts para que possamos usar métodos do banco nela.
}
